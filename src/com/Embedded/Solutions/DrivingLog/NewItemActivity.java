 ...
 *                                                                             *
 *   Driving Log is free software;  you can  redistribute it                   *
 *   and/or modify it under terms of the  GNU General Public License           *
 *   as published  by the Free Software  Foundation;  either version 2,        *
 *   or (at your option) any later version.                                    *
 *   Driving Log is distributed in the hope that it will be                    *
...
public class NewItemActivity extends Activity implements Button.OnClickListener {
	...
	public void onClick(View v) {
		...
		if (v == m_toggleButton) {
			...
			if (m_running) {
				if (m_tripStopField.getText().toString().length() == 0) {
					Toast toast = Toast.makeText(getApplicationContext(), Lang
							.get(Lang.FILLIN_TRIP_STOP), Toast.LENGTH_SHORT);
					toast.show();
					return;
				}
				// FM: Stop tid > start tid
				long tripEnd = Long.parseLong(m_tripStopField.getText()
						.toString());
				...
				if (m_tripStartField.getText().toString().length() == 0) {
					Toast toast = Toast.makeText(getApplicationContext(), Lang
							.get(Lang.FILLIN_TRIP_START), Toast.LENGTH_SHORT);
					toast.show();
					return;
				}

				if (m_descriptionField.getText().toString().contains(";")) {
					Toast toast = Toast.makeText(getApplicationContext(), Lang
							.get(Lang.NO_SEMICOLON), Toast.LENGTH_SHORT);
					toast.show();
					return;
				...
				if (!Config.isFullVersion()) {
					...
					if (c.getCount() >= 5) {
						GenericAlertMessage.alert(this, Lang.get(Lang.MAX_RECORDS_IN_DEMO_VERSION));
						ds.close();
						return;
				...
				long tripStart = Long.parseLong(m_tripStartField.getText()
						.toString());
				ds.insertItem(type, System.currentTimeMillis(),
						m_descriptionField.getText().toString(), tripStart);
				ds.close();
				toggleStatus();
	...
	private void setState() {
		if (m_running == true) { // Recording:
            ...
            PorterDuffColorFilter filter = new PorterDuffColorFilter(Color.argb(100, 200, 0, 0), PorterDuff.Mode.SRC_ATOP);  
	...
	private Boolean initWithRunning() {
		...
		if (!c.moveToFirst()) {
			...
			if (c.getString(c.getColumnIndex(Const.KEY_TYPE)).equals(
			...
			long startTime = c.getLong(c.getColumnIndex(Const.KEY_START_TIME));
			ds.close();

			Toast toast = Toast
					.makeText(getApplicationContext(), Lang
							.get(Lang.RECORDING_STARTED)
							+ DateTimeFormat.getTime(startTime) + Lang.get(Lang.CONTINUE),
							Toast.LENGTH_LONG);
